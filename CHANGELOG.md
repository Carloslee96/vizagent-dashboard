...
### 文档：主题展示章节标题与 25 主题特性对齐

- 修首页矛盾：v0.1.7 特性 bullet 写「25 个主题」，但下方展示章节标题仍是「同一份数据，5 个主题」（只展示 5 个原创主题截图），让人误以为总共只有 5 个。
- README（中/英）章节标题改为「同一份数据，25 个主题任选」；预览下方注明「展示 5 个原创主题预览，另 20 个去品牌主题见 `docs/THEME_AUDIT.md`，用 `--theme <id>` 切换」，点名真实存在的 `coral-warm`/`grove-dark`/`phosphor-green` 三个示例。
- 纯文档变更，无代码改动；PyPI 首页 README 同步。
